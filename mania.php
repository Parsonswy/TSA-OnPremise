	...
	<body>
       ...
       <p class="links">
		...
		<?php	
			...
			<form action='./mania.php' method='post'>
						<table>
							<tr>
							...
							<tr>
								<td><font class='blind'>Boxes:</td>
								<td><select  name='tickets'> 
									<option value='1'>1 Boxes  $5</option> 
									<option value='2'>2 Boxes  $10</option>
									<option value='4'>4 Boxes $20</option> 
									</select></td>
							</tr>
							<tr>
								<td></td>
		...
		if(ISSET($_POST['charge'])){
			...
			$get_info = "Mystery Mania || $get_ticket tickets || $get_user : Account";
				if($get_user){
					if($get_pin){
						if($get_ticket){
							if($get_ticket  == 1){}
							elseif($get_ticket == 2){$price = $price * 2;}
							elseif($get_ticket == 4){$price = $price * 4;}
							else
