...
if($query->num_rows){
	...
	}else{$name = $mysqli->real_escape_string(qturn['name']); die("($name)UUID $uuid has no charges! $query->num_rows" . $qturn['balance']);}
...
if(ISSET($uuid) && !ISSET($_GET['lkpID'])){
	...
	if($query->num_rows){
		...
		$query = $mysqli->query("SELECT * FROM `transactions` WHERE `uuid`=$uuid && `completed`=1");
		while($rows = $query->fetch_assoc()){
			$pdf->Cell(130, 10, $rows["log"],1,0,"L",0);
			$pdf->Cell(60, 10, $rows["cost"], 1,1,"C",0);
		}
		...
		$pdf->Cell(0,0, "Click Here To Confirm", 1, 0, "C", 0, "http://" . $_SERVER["SERVER_ADDR"] . "/2016/TSA/cashOut.php?complete=$transID&lkpID=$lkptime&uuid=$uuid&alternate=$alt");
	...
	if($query->num_rows){
		...
		if($query->num_rows){
			$qturn = $query->fetch_assoc();
			$name = $mysqli->real_escape_string($qturn['name']);
			$query = $mysqli->query("UPDATE `transactions` SET completed=3 WHERE id=$transID AND lkpID=$lkpID");
